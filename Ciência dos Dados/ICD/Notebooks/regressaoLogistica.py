...
Original file is located at
    https://colab.research.google.com/drive/1Xi5FwjjcgIx_dEqI42MveXiGK8QHtE_f
...
def cross_entropy_mean(X, y, theta):
    logit = sigmoid(np.dot(X, theta))  # Produto X @ theta seguido da sigmoide
    logit = np.clip(logit, 1e-5, 1 - 1e-5)  # Evitar log(0)
...
def derivadas(X, y, theta):
    ...
    predictions = sigmoid(np.dot(X, theta))  # f_θ(X)
    gradient = -1/m * np.dot(X.T, (y - predictions))  # Vetorização de L'(θ)
    return gradient
...
def gd(X, y, lambda_=0.01, tol=1e-7, max_iter=10000):
    ...
    while True:
        ...
        grad = -1 / X.shape[0] * np.dot(X.T, (y - predictions))

        theta_novo = theta - lambda_ * grad

        err = sol.cross_entropy_mean(X, y, theta)
        ...
        if i == max_iter:
            print("Atingido numero maximo de iteracoes.")
            break
