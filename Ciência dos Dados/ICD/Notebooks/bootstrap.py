...
def CI(data):
    ...
    data2 = data.query('DEPENDENCIA_ADMINISTRATIVA == "Estadual"')
    data2 = data2.dropna(subset=['NOTA_MEDIA_ESCOLA'])

    mean = data2['NOTA_MEDIA_ESCOLA'].mean()
    std = data2['NOTA_MEDIA_ESCOLA'].std()

    tam = len(data2)
    inver_norm = ss.norm.ppf(1 - 0.01 / 2)
    erro_std = std / np.sqrt(tam)

    LI = mean - inver_norm * erro_std
    LS = mean + inver_norm * erro_std
    return round(LI, 2), round(LS, 2)
...
plt.plot(ecdf.x, ecdf.y, color="blue", lw=2)
plt.title("Função de Distribuição Acumulada (CDF)", fontsize=16)
plt.xlabel("TAXA_DE_APROVACAO", fontsize=12)
