...
def percent(df1, df2, alpha):
    ...
    median1 = df1['TAXA_DE_PARTICIPACAO'].median()
    median2 = df2['TAXA_DE_PARTICIPACAO'].median()
    observed_diff = abs(median1 - median2)

    combined = np.concatenate([df1['TAXA_DE_PARTICIPACAO'], df2['TAXA_DE_PARTICIPACAO']])
    ...
    for _ in range(10000):
        ...
        sample1 = combined[:n1]
        sample2 = combined[n1:]
        boot_diffs.append(abs(np.median(sample1) - np.median(sample2)))

    lower_percentile = np.percentile(boot_diffs, alpha / 2 * 100)
    upper_percentile = np.percentile(boot_diffs, (1 - alpha / 2) * 100)

    if observed_diff < lower_percentile or observed_diff > upper_percentile:
        return True
    return False
...
def t_obs(data):
  ...
  mean1 = data[data['TEAM'] == 'Charlotte Hornets']['SALARY'].mean()
  mean2 = data[data['TEAM'] == 'Phoenix Suns']['SALARY'].mean()
...
def resposta(data):
  ...
  percentil_inf1 = mean1 * (0.1 / 2) * 100
  percentil_sup1 = mean1 * (1 - (0.1 / 2)) * 100
  percentil_inf2 = mean2 * (0.1 / 2) * 100
  percentil_sup2 = mean2 * (1 - (0.1 / 2)) * 100

  if(percentil_inf1 > percentil_sup2) | (percentil_sup1 < percentil_inf2):
    return False
  else:
    return True
...
def t_obs2(data):
    # Separamos as alturas dos jogadores para os dois times
    cleveland_heights = data[data['team_abbreviation'] == 'CLE']['player_height']
    golden_state_heights = data[data['team_abbreviation'] == 'GSW']['player_height']

    # Calculamos a diferença de médias entre os dois times
    t_statistic = np.mean(cleveland_heights) - np.mean(golden_state_heights)
...
valores_criticos = critical_values(diffs)
...
def p_value(t_obs, diffs):
  ...
  superiores = [x for x in sort if x <= t_obs]
  p_valor = len(superiores) / len(diffs)
