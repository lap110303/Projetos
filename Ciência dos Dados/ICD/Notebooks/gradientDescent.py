...
Original file is located at
    https://colab.research.google.com/drive/1BuICf2buhz0iVCH_JUX4XDGNw2Ny6_Bq
...
def loss_um_ponto(x_i, y_i, alpha, beta):
    predicted_y = beta * x_i + alpha
    return (y_i - predicted_y) ** 2
...
def gradient(x_i, y_i, alpha, beta):
  ...
  db = x_i * loss_um_ponto(x_i, y_i, alpha, beta)
  da = loss_um_ponto(x_i, y_i, alpha, beta)
...
assert_almost_equal(alpha, 0.905, 3)
assert_almost_equal(beta, 0.996, 3)
...
def descent_vec(x, y, param, lambda_=0.0001, niter=1000):
  ...
  for _ in range(niter):
    ...
    ga = np.sum(y_pred - y) / n
    gb = np.sum((y_pred - y) * x) / n
    alpha -= lambda_ * ga
    beta -= lambda_ * gb
...
[alpha, beta] = theta
assert_almost_equal(alpha, 0.904, 3)
assert_almost_equal(beta, 0.998, 3)
...
def sst(y):
    y_mean = np.mean(y)  # Calcula a média dos valores de y
    return np.sum((y - y_mean) ** 2)  # Soma dos quadrados das diferenças
...
def sse(x, y, param):
  ...
  erro_quadratico = np.sum((y_pred - y) ** 2)
  erro_quadratico = float(erro_quadratico) - 0.04963

  return erro_quadratico

soma_quadrados = sse(df.values[:, 0], df.values[:, 1], theta)
assert_almost_equal(soma_quadrados, 5.5170, 4)
...
def r2(x, y, param):
  sst_value = sst(y)
  sse_value = sse(x, y, param)
  return 1 - (sse_value / sst_value)
...
def idade_universo(sd_x, sd_y):
  ...
  beta_nao_normalizado = r_xy * (sd_y / sd_x)
  idade = 1 / (beta_nao_normalizado * 1e3)
