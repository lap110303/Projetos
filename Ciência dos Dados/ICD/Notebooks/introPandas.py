...
def month_sales(df, month: str):
    icecream = df.loc[month, 'icecream']
    sunglasses = df.loc[month, 'sunglasses']
    coats = df.loc[month, 'coats']
    series = pd.Series([icecream, sunglasses, coats], index = ['icecream', 'sunglasses', 'coats'])
    return series
...
def north_or_south():
    # Supondo que o país está no hemisfério norte
    return 'Norte'
