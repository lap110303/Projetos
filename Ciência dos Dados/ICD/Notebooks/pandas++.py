...
assert_almost_equal(10.456614371849216, delay(df_clean)[0])
assert_almost_equal(6.563286883449304, delay(df_clean)[1])
...
def high_delay(df):
    ...
    tempo_medio_rota = good.groupby(['OriginAirportName', 'DestAirportName'])['Q5'].mean()

    maior_rota = tempo_medio_rota.idxmax()
...
# Criar um boxplot dos atrasos de saída por dia da semana
plt.figure(figsize=(10, 6))
sns.boxplot(x='DayOfWeek', y='DepDelay', data=df, showfliers=False,
            boxprops=dict(facecolor='none', edgecolor='blue'))  # Sem cor interna, borda azul
...
correlation = df_filtered['DepDelay'].corr(df_filtered['ArrDelay'])

# Criar gráfico de dispersão
plt.figure(figsize=(10, 6))
plt.scatter(df_filtered['DepDelay'], df_filtered['ArrDelay'], alpha=0.5, s=30)  # s para o tamanho dos pontos
plt.title('Gráfico de Dispersão: Atraso na Partida vs Atraso na Chegada')
plt.xlabel('ArrDelay')
plt.ylabel('DeoDelay')
