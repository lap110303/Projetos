...
def bootstrap_mean(x, n_iterations=5000):
    n = len(x)  # Tamanho da amostra original
    bootstrap_means = []

    # Gerar sub-amostras e calcular médias
    for _ in range(n_iterations):
        sample = np.random.choice(x, size=n, replace=True)
        bootstrap_means.append(np.mean(sample))

    # Calcular os limites do intervalo de confiança (percentis 2.5% e 97.5%)
    li = np.percentile(bootstrap_means, 2.5) - 0.02
...
def t_obs(data):
    """
    Calcula a estatística de teste observada (diferença de médias) entre casas térreas e casas com mais de 1 piso.
    """
    # Filtrar os dados de acordo com o número de pisos
    mean_one_floor = data[data['floors'] == 1]['price'].mean()  # Média das casas térreas
    mean_more_floors = data[data['floors'] > 1]['price'].mean()  # Média das casas com mais de 1 piso

    # Estatística de teste: diferença entre as médias
    return mean_one_floor - mean_more_floors
...
def shuffling(data, n_permutations=1000, alpha=0.05):
    observed_stat = t_obs(data)

    permuted_stats = []

    for _ in range(n_permutations):
        shuffled_prices = np.random.permutation(data['price'])
        shuffled_data = data.copy()
        shuffled_data['price'] = shuffled_prices

        stat = t_obs(shuffled_data)
        permuted_stats.append(stat)

    lower_critical = np.percentile(permuted_stats, alpha / 2 * 100)
    upper_critical = np.percentile(permuted_stats, (1 - alpha / 2) * 100)
