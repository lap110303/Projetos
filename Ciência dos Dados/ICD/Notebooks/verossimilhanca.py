...
def r_squared(alpha, beta, x, y):
    ss_res = sum_of_squared_errors(alpha, beta, x, y)
    ss_tot = total_sum_of_squares(y)
...
sns.residplot(x=x, y=y, lowess=True, line_kws={'color': 'red', 'lw': 2})
...
def error_verification(alpha, beta, x, y):
    ...
    greater_than_zero = np.sum(residuals > 0)
    less_than_zero = np.sum(residuals < 0)
...
# Gráfico de Probabilidade (QQ Plot)
plt.figure(figsize=(10, 6))
stats.probplot(y - y.mean(), plot=plt)  # Resíduos centrados
sns.despine()  # Remove bordas superiores e laterais
plt.title('Gráfico de Probabilidade (QQ Plot)', fontsize=14)
plt.show()
...
sns.residplot(x=x, y=y, lowess=True, line_kws={'color': 'red', 'lw': 2})
plt.axhline(0, color='blue', linestyle='--', linewidth=1)
plt.title('Gráfico de Resíduos - Stocks and Unemployment')
...
sns.despine()
plt.title('QQ Plot - Resíduos do Modelo')
plt.show()
...
sns.residplot(x=x, y=y, lowess=True, line_kws={'color': 'red', 'lw': 2})
plt.axhline(0, color='blue', linestyle='--', linewidth=1)
plt.title('Gráfico de Resíduos - Stocks and Interest')
...
plt.figure(figsize=(10, 6))
stats.probplot(residuals, plot=plt)  # QQ plot para os resíduos
sns.despine()
plt.title('QQ Plot - Resíduos do Modelo')
plt.show()
...
sns.residplot(x=x, y=y, lowess=True, line_kws={'color': 'red', 'lw': 2})
plt.axhline(0, color='blue', linestyle='--', linewidth=1)
plt.title('Gráfico de Resíduos - Dugongs')
plt.xlabel('Age')
plt.ylabel('Resíduos')
plt.show()
